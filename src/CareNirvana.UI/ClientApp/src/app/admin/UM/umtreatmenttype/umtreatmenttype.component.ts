...
@Component({
  selector: 'app-umtreatmenttype',
  templateUrl: './umtreatmenttype.component.html',
  styleUrl: './umtreatmenttype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmtreatmenttypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'treatmentType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'treatmentType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      treatmentType: '', // Empty input for Treatment Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.treatmentType}"?`
      : `Are you sure you want to inactivate the "${row.treatmentType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'treatmenttype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Treatment Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Treatment Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Treatment Type:', row.treatmentType);

    // Check if Treatment Type is empty
    if (!row.treatmentType) {
      this.snackBar.open('Treatment Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Treatment Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.treatmentType?.toLowerCase() === row.treatmentType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Treatment Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'treatmenttype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Treatment Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'treatmenttype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Treatment Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.treatmentType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'treatmenttype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Treatment Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Treatment Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
