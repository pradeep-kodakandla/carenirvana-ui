...
@Component({
  selector: 'app-umdischargetype',
  templateUrl: './umdischargetype.component.html',
  styleUrl: './umdischargetype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmdischargetypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'dischargeType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'dischargetype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'dischargeType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      dischargeType: '', // Empty input for Discharge Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.dischargeType}"?`
      : `Are you sure you want to inactivate the "${row.dischargeType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'dischargetype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Discharge Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Discharge Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Discharge Type:', row.dischargeType);

    // Check if Discharge Type is empty
    if (!row.dischargeType) {
      this.snackBar.open('Discharge Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Discharge Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.dischargeType?.toLowerCase() === row.dischargeType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Discharge Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'dischargetype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Discharge Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'dischargetype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Discharge Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.dischargeType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'dischargetype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Discharge Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Discharge Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
