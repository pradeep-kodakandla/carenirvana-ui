...
@Component({
  selector: 'app-umoutofareaindicator',
  templateUrl: './umoutofareaindicator.component.html',
  styleUrl: './umoutofareaindicator.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmoutofareaindicatorComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'outOfAreaIndicator', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'outofareaindicator').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'outOfAreaIndicator', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      outOfAreaIndicator: '', // Empty input for Out Of Area Indicator
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.outOfAreaIndicator}"?`
      : `Are you sure you want to inactivate the "${row.outOfAreaIndicator}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'outofareaindicator', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Out Of Area Indicator updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Out Of Area Indicator!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Out Of Area Indicator:', row.outOfAreaIndicator);

    // Check if Out Of Area Indicator is empty
    if (!row.outOfAreaIndicator) {
      this.snackBar.open('Out Of Area Indicator is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Out Of Area Indicator
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.outOfAreaIndicator?.toLowerCase() === row.outOfAreaIndicator.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Out Of Area Indicator already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'outofareaindicator', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Out Of Area Indicator added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'outofareaindicator', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Out Of Area Indicator updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.outOfAreaIndicator}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'outofareaindicator', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Out Of Area Indicator deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Out Of Area Indicator!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
