...
@Component({
  selector: 'app-umnotetype',
  templateUrl: './umnotetype.component.html',
  styleUrl: './umnotetype.component.css'
})
export class UmnotetypeComponent implements OnInit {
  ...
  saveEntry() {
    ...
    if (this.formMode === 'add') {
      ...
      // Save new entry
      this.crudService.addData('um', 'notetype', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('um', 'notetype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    if (confirm(message)) {
      ...
      this.crudService.updateData('um', 'notetype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Note Type updated successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'notetype', id, 'current_user').subscribe(
      () => {
