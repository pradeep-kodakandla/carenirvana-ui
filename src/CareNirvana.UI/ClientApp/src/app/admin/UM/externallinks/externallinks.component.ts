...
@Component({
  ...
  styleUrl: './externallinks.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class ExternallinksComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'name', 'URL', 'activeFlag', 'isProviderPortal', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'externallinks').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'name', 'URL', 'activeFlag','isProviderPortal', 'createdBy', 'createdOn', 'actions'];
  ...
  onRowDoubleClick(row: any, event: MouseEvent) {
    ...
    event.stopPropagation(); // Prevent event from propagating to the External links
  ...
  addRow(): void {
    const newRow = {
      ...
      name: '', // Empty input for External links
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.name}"?`
      : `Are you sure you want to inactivate the "${row.name}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'externallinks', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('External link updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating External link!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveIsProviderPortal(event: MatCheckboxChange, row: any): void {
    // Update the element's activeFlag value based on the checkbox status
    const isChecked = event.checked;
    const message = isChecked
      ? `Are you sure you want to activate the "${row.name}"?`
    ...
    if (confirm(message)) {
      row.isProviderPortal = event.checked;
  ...
  saveEntry(row: any) {
    ...
    // Check if External LinkType is empty
    if (!row.name) {
      this.snackBar.open('External link is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate External LinkType
    ...
    if (isDuplicate) {
      this.snackBar.open('External Link already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'externallinks', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('External Link added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'externallinks', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('External Link updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.name}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'externallinks', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('External Link deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting External Link!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
