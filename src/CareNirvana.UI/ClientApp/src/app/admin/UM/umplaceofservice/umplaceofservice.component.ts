...
@Component({
  selector: 'app-umplaceofservice',
  templateUrl: './umplaceofservice.component.html',
  styleUrl: './umplaceofservice.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmplaceofserviceComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'placeOfServiceCode', 'placeOfServiceName', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'placeOfServiceCode', 'placeOfServiceName', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      placeOfServiceCode: '', // Empty input for Place Of Service Code
      placeOfServiceName: '', // Empty input for Place Of Service Name
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.placeOfServiceName}"?`
      : `Are you sure you want to inactivate the "${row.placeOfServiceName}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'placeofservice', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Place Of Service updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Place Of Service!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    // Check if Place Of Service is empty
    if (!row.placeOfServiceCode) {
      this.snackBar.open('Place Of Service Code is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (!row.placeOfServiceName) {
      this.snackBar.open('Place Of Service Name is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Place Of Service
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.placeOfServiceName?.toLowerCase() === row.placeOfServiceName.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Place Of Service already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'placeofservice', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Place Of Service added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'placeofservice', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Place Of Service updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.placeOfServiceName}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'placeofservice', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Place Of Service deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Place Of Service!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
