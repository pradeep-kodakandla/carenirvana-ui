...
@Component({
  selector: 'app-umunittype',
  templateUrl: './umunittype.component.html',
  styleUrl: './umunittype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmunittypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'unitType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'unitType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      unitType: '', // Empty input for Unit Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.unitType}"?`
      : `Are you sure you want to inactivate the "${row.unitType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'unittype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Unit Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Unit Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Unit Type:', row.unitType);

    // Check if Unit Type is empty
    if (!row.unitType) {
      this.snackBar.open('Unit Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.unitType?.toLowerCase() === row.unitType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Unit Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'unittype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Unit Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'unittype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Unit Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.unitType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'unittype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Unit Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Unit Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
