...
@Component({
  selector: 'app-umdischargeto',
  templateUrl: './umdischargeto.component.html',
  styleUrl: './umdischargeto.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmdischargetoComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'dischargeTo', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'dischargeto').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'dischargeTo', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      dischargeTo: '', // Empty input for Discharge To
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.dischargeTo}"?`
      : `Are you sure you want to inactivate the "${row.dischargeTo}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'dischargeto', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Discharge To updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Discharge To!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Discharge To:', row.dischargeTo);

    // Check if Discharge To is empty
    if (!row.dischargeTo) {
      this.snackBar.open('Discharge To is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Discharge To
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.dischargeTo?.toLowerCase() === row.dischargeTo.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Discharge To already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'dischargeto', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Discharge To added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'dischargeto', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Discharge To updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.dischargeTo}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'dischargeto', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Discharge To deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Discharge To!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
