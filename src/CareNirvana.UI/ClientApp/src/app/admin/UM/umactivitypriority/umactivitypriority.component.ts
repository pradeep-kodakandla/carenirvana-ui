...
@Component({
  selector: 'app-umactivitypriority',
  templateUrl: './umactivitypriority.component.html',
  styleUrl: './umactivitypriority.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmactivitypriorityComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'activityPriority', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'activitypriority').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'activityPriority', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      activityPriority: '', // Empty input for Activity Priority
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.activityPriority}"?`
      : `Are you sure you want to inactivate the "${row.activityPriority}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'activitypriority', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Activity Priority updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Activity Priority!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Activity Priority:', row.activityPriority);

    // Check if Activity Priority is empty
    if (!row.activityPriority) {
      this.snackBar.open('Activity Priority is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Activity Priority
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.activityPriority?.toLowerCase() === row.activityPriority.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Activity Priority already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'activitypriority', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Activity Priority added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'activitypriority', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Activity Priority updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.activityPriority}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'activitypriority', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Activity Priority deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Activity Priority!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
