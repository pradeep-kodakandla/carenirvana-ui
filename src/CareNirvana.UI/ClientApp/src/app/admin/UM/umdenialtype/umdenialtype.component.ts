...
@Component({
  selector: 'app-umdenialtype',
  templateUrl: './umdenialtype.component.html',
  styleUrl: './umdenialtype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmdenialtypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'denialType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'denialtype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'denialType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      denialType: '', // Empty input for Denial Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.denialType}"?`
      : `Are you sure you want to inactivate the "${row.denialType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'denialtype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Denial Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Denial Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Denial Type:', row.denialType);

    // Check if Denial Type is empty
    if (!row.denialType) {
      this.snackBar.open('Denial Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Denial Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.denialType?.toLowerCase() === row.denialType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Denial Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'denialtype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Denial Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'denialtype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Denial Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.denialType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'denialtype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Denial Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Denial Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
