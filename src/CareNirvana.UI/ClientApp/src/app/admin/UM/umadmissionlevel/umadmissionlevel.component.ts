...
@Component({
  selector: 'app-umadmissionlevel',  
  templateUrl: './umadmissionlevel.component.html',
  styleUrl: './umadmissionlevel.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmadmissionlevelComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'admissionLevel', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'admissionlevel').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'admissionLevel', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      admissionLevel: '', // Empty input for Admission Level
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.admissionLevel}"?`
      : `Are you sure you want to inactivate the "${row.admissionLevel}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'admissionlevel', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Admission Level updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Admission Level!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Admission Level:', row.admissionLevel);

    // Check if Admission Level is empty
    if (!row.admissionLevel) {
      this.snackBar.open('Admission Level is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Admission Level
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.admissionLevel?.toLowerCase() === row.admissionLevel.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Admission Level already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'admissionlevel', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Admission Level added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'admissionlevel', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Admission Level updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.admissionLevel}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'admissionlevel', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Admission Level deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Admission Level!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
