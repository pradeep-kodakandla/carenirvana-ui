...
@Component({
  selector: 'app-umprescriptionquantity',
  templateUrl: './umprescriptionquantity.component.html',
  styleUrl: './umprescriptionquantity.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmprescriptionquantityComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'prescriptionQuantity', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'prescriptionquantity').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'prescriptionQuantity', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      prescriptionQuantity: '', // Empty input for Prescription Quantity
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.prescriptionQuantity}"?`
      : `Are you sure you want to inactivate the "${row.prescriptionQuantity}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'prescriptionquantity', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Prescription Quantity updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Prescription Quantity!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Prescription Quantity:', row.prescriptionQuantity);

    // Check if Prescription Quantity is empty
    if (!row.prescriptionQuantity) {
      this.snackBar.open('Prescription Quantity is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Prescription Quantity
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.prescriptionQuantity?.toLowerCase() === row.prescriptionQuantity.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Prescription Quantity already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'prescriptionquantity', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Prescription Quantity added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'prescriptionquantity', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Prescription Quantity updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.prescriptionQuantity}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'prescriptionquantity', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Prescription Quantity deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Prescription Quantity!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
