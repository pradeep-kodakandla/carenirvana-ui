...
@Component({
  selector: 'app-umadmitreason',
  templateUrl: './umadmitreason.component.html',
  styleUrl: './umadmitreason.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmadmitreasonComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'admitReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'admitreason').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'admitReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      admitReason: '', // Empty input for Admit Reason
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.admitReason}"?`
      : `Are you sure you want to inactivate the "${row.admitReason}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'admitreason', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Admit Reason updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Admit Reason!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Admit Reason:', row.admitReason);

    // Check if Admit Reason is empty
    if (!row.admitReason) {
      this.snackBar.open('Admit Reason is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Admit Reason
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.admitReason?.toLowerCase() === row.admitReason.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Admit Reason already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'admitreason', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Admit Reason added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'admitreason', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Admit Reason updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.admitReason}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'admitreason', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Admit Reason deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Admit Reason!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
