...
@Component({
  selector: 'app-umtransportationtriptype',
  templateUrl: './umtransportationtriptype.component.html',
  styleUrl: './umtransportationtriptype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmtransportationtriptypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'transportationTripType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'transportationtriptype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'transportationTripType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      transportationTripType: '', // Empty input for Transportation Trip Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.transportationTripType}"?`
      : `Are you sure you want to inactivate the "${row.transportationTripType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'transportationtriptype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Transportation Trip Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Transportation Trip Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Transportation Trip Type:', row.transportationTripType);

    // Check if Transportation Trip Type is empty
    if (!row.transportationTripType) {
      this.snackBar.open('Transportation Trip Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Transportation Trip Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.transportationTripType?.toLowerCase() === row.transportationTripType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Transportation Trip Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'transportationtriptype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Transportation Trip Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'transportationtriptype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Transportation Trip Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.transportationTripType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'transportationtriptype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Transportation Trip Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Transportation Trip Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
