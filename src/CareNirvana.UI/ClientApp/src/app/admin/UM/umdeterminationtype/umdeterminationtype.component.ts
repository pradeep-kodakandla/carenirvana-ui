...
@Component({
  selector: 'app-umdeterminationtype',
  templateUrl: './umdeterminationtype.component.html',
  styleUrl: './umdeterminationtype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmdeterminationtypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'determinationType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'determinationtype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'determinationType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      determinationType: '', // Empty input for Determination Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.determinationType}"?`
      : `Are you sure you want to inactivate the "${row.determinationType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'determinationtype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Determination Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Determination Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Determination Type:', row.determinationType);

    // Check if Determination Type is empty
    if (!row.determinationType) {
      this.snackBar.open('Determination Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Determination Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.determinationType?.toLowerCase() === row.determinationType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Determination Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'determinationtype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Determination Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'determinationtype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Determination Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.determinationType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'determinationtype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Determination Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Determination Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
