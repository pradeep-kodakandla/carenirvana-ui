...
@Component({
  selector: 'app-ummedicationfrequency',
  templateUrl: './ummedicationfrequency.component.html',
  styleUrl: './ummedicationfrequency.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmmedicationfrequencyComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'medicationFrequency', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'medicationfrequency').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'medicationFrequency', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      medicationFrequency: '', // Empty input for Medication Frequency
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.medicationFrequency}"?`
      : `Are you sure you want to inactivate the "${row.medicationFrequency}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'medicationfrequency', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Medication Frequency updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Medication Frequency!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Medication Frequency:', row.medicationFrequency);

    // Check if Medication Frequency is empty
    if (!row.medicationFrequency) {
      this.snackBar.open('Medication Frequency is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Medication Frequency
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.medicationFrequency?.toLowerCase() === row.medicationFrequency.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Medication Frequency already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'medicationfrequency', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Frequency added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('um', 'medicationfrequency', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Frequency updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.medicationFrequency}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'medicationfrequency', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Medication Frequency deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Medication Frequency!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
