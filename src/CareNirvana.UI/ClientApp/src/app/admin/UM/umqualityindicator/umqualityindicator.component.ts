...
@Component({
  selector: 'app-umqualityindicator',
  templateUrl: './umqualityindicator.component.html',
  styleUrl: './umqualityindicator.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class UmqualityindicatorComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'qualityIndicator', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('um', 'qualityindicator').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'qualityIndicator', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      qualityIndicator: '', // Empty input for Quality Indicator
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.qualityIndicator}"?`
      : `Are you sure you want to inactivate the "${row.qualityIndicator}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('um', 'qualityindicator', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Quality Indicator updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Quality Indicator!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Quality Indicator:', row.qualityIndicator);

    // Check if Quality Indicator is empty
    if (!row.qualityIndicator) {
      this.snackBar.open('Quality Indicator is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Quality Indicator
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.qualityIndicator?.toLowerCase() === row.qualityIndicator.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Quality Indicator already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('um', 'qualityindicator', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Quality Indicator added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      console.log('EditValue:', JSON.stringify(this.selectedEntry, null, 2));
      if (this.selectedEntry && this.selectedEntry.id != null) {
        this.selectedEntry.id = this.selectedEntry.id.toString(); // Safely convert to string
      } else {
        console.error('Invalid id:', this.selectedEntry?.id);
      }
      this.crudService.updateData('um', 'qualityindicator', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Quality Indicator updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.qualityIndicator}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('um', 'qualityindicator', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Quality Indicator deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Quality Indicator!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
