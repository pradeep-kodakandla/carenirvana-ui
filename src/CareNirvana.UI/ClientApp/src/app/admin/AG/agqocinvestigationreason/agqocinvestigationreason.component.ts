...
@Component({
  selector: 'app-agqocinvestigationreason',  
  templateUrl: './agqocinvestigationreason.component.html',
  styleUrl: './agqocinvestigationreason.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgqocinvestigationreasonComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'qocInvestigationReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'qocinvestigationreason').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'qocInvestigationReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      qocInvestigationReason: '', // Empty input for QOC Investigation Reason
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.qocInvestigationReason}"?`
      : `Are you sure you want to inactivate the "${row.qocInvestigationReason}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'qocinvestigationreason', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('QOC Investigation Reason updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating QOC Investigation Reason!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('QOC Investigation Reason:', row.qocInvestigationReason);

    // Check if QOC Investigation Reason is empty
    if (!row.qocInvestigationReason) {
      this.snackBar.open('QOC Investigation Reason is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate QOC Investigation Reason
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.qocInvestigationReason?.toLowerCase() === row.qocInvestigationReason.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('QOC Investigation Reason already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'qocinvestigationreason', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Investigation Reason added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'qocinvestigationreason', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Investigation Reason updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.qocInvestigationReason}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'qocinvestigationreason', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('QOC Investigation Reason deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting QOC Investigation Reason!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
