...
@Component({
  selector: 'app-agcomplaintclass',  
  templateUrl: './agcomplaintclass.component.html',
  styleUrl: './agcomplaintclass.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcomplaintclassComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'complaintClass', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'complaintclass').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'complaintClass', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      complaintClass: '', // Empty input for Complaint Class
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.complaintClass}"?`
      : `Are you sure you want to inactivate the "${row.complaintClass}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'complaintclass', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Complaint Class updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Complaint Class!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Complaint Class:', row.complaintClass);

    // Check if Complaint Class is empty
    if (!row.complaintClass) {
      this.snackBar.open('Complaint Class is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Complaint Class
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.complaintClass?.toLowerCase() === row.complaintClass.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Complaint Class already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'complaintclass', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Class added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'complaintclass', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Class updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.complaintClass}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'complaintclass', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Complaint Class deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Complaint Class!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
