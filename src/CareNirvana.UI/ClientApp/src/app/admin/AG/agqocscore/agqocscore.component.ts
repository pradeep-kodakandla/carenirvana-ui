...
@Component({
  selector: 'app-agqocscore',  
  templateUrl: './agqocscore.component.html',
  styleUrl: './agqocscore.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgqocscoreComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'qocScore', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'qocscore').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'qocScore', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      qocScore: '', // Empty input for QOC Score
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.qocScore}"?`
      : `Are you sure you want to inactivate the "${row.qocScore}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'qocscore', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('QOC Score updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating QOC Score!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('QOC Score:', row.qocScore);

    // Check if QOC Score is empty
    if (!row.qocScore) {
      this.snackBar.open('QOC Score is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate QOC Score
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.qocScore?.toLowerCase() === row.qocScore.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('QOC Score already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'qocscore', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Score added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'qocscore', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Score updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.qocScore}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'qocscore', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('QOC Score deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting QOC Score!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
