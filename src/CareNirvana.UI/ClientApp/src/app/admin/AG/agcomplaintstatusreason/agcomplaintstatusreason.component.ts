...
@Component({
  selector: 'app-agcomplaintstatusreason',  
  templateUrl: './agcomplaintstatusreason.component.html',
  styleUrl: './agcomplaintstatusreason.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcomplaintstatusreasonComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'complaintStatusReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'complaintstatusreason').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'complaintStatusReason', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      complaintStatusReason: '', // Empty input for Complaint Status Reason
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.complaintStatusReason}"?`
      : `Are you sure you want to inactivate the "${row.complaintStatusReason}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'complaintstatusreason', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Complaint Status Reason updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Complaint Status Reason!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Complaint Status Reason:', row.complaintStatusReason);

    // Check if Complaint Status Reason is empty
    if (!row.complaintStatusReason) {
      this.snackBar.open('Complaint Status Reason is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Complaint Status Reason
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.complaintStatusReason?.toLowerCase() === row.complaintStatusReason.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Complaint Status Reason already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'complaintstatusreason', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Status Reason added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'complaintstatusreason', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Status Reason updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.complaintStatusReason}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'complaintstatusreason', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Complaint Status Reason deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Complaint Status Reason!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
