...
@Component({
  selector: 'app-agdocumenttype',
  templateUrl: './agdocumenttype.component.html',
  styleUrl: './agdocumenttype.component.css'
})
export class AgdocumenttypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'documentType', 'documentTemplate', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'documenttype').subscribe((response) => {
  ...
  openForm(mode: 'add' | 'edit' | 'view', element: any = null) {
    ...
    this.formMode = mode;
    this.selectedEntry = element ? { ...element } : { id: null, documentType: '', activeFlag: false, documentTemplate: '' };
  ...
  saveEntry() {
    ...
    // Check if Document Type is empty
    if (!formValue.documentType) {
      this.snackBar.open('Document Type is required!', 'Close', {
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.documentType?.toLowerCase() === formValue.documentType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Document Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      // Save new entry
      this.crudService.addData('ag', 'documenttype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Document Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 50000,
      ...
      this.crudService.updateData('ag', 'documenttype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Document Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 50000,
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.documentType}"?`
      : `Are you sure you want to inactivate the "${row.documentType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'documenttype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Document Type updated successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Document Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the document "${element.documentType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'documenttype', id, 'current_user').subscribe(
      () => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'documentType', 'documentTemplate', 'activeFlag', 'createdBy', 'createdOn', 'actions'];;
