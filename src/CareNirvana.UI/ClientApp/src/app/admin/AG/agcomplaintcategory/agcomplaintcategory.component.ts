...
@Component({
  selector: 'app-agcomplaintcategory',  
  templateUrl: './agcomplaintcategory.component.html',
  styleUrl: './agcomplaintcategory.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcomplaintcategoryComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'complaintCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'complaintcategory').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'complaintCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      complaintCategory: '', // Empty input for Complaint Category
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.complaintCategory}"?`
      : `Are you sure you want to inactivate the "${row.complaintCategory}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'complaintcategory', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Complaint Category updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Complaint Category!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Complaint Category:', row.complaintCategory);

    // Check if Complaint Category is empty
    if (!row.complaintCategory) {
      this.snackBar.open('Complaint Category is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Complaint Category
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.complaintCategory?.toLowerCase() === row.complaintCategory.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Complaint Category already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'complaintcategory', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Category added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'complaintcategory', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Category updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.complaintCategory}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'complaintcategory', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Complaint Category deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Complaint Category!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
