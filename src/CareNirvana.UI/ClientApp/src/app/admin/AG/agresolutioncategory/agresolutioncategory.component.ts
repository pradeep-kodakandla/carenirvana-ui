...
@Component({
  selector: 'app-agresolutioncategory',
  templateUrl: './agresolutioncategory.component.html',
  styleUrl: './agresolutioncategory.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgresolutioncategoryComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'resolutionCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'resolutioncategory').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'resolutionCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      resolutionCategory: '', // Empty input for Resolution Category
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.resolutionCategory}"?`
      : `Are you sure you want to inactivate the "${row.resolutionCategory}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'resolutioncategory', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Resolution Category updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Resolution Category!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Resolution Category:', row.resolutionCategory);

    // Check if Resolution Category is empty
    if (!row.resolutionCategory) {
      this.snackBar.open('Resolution Category is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Resolution Category
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.resolutionCategory?.toLowerCase() === row.resolutionCategory.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Resolution Category already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'resolutioncategory', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Resolution Category added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'resolutioncategory', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Resolution Category updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.resolutionCategory}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'resolutioncategory', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Resolution Category deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Resolution Category!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
