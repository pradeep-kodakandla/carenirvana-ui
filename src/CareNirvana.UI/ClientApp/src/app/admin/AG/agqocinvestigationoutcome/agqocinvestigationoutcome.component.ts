...
@Component({
  selector: 'app-agqocinvestigationoutcome',  
  templateUrl: './agqocinvestigationoutcome.component.html',
  styleUrl: './agqocinvestigationoutcome.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgqocinvestigationoutcomeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'qocInvestigationOutcome', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'qocinvestigationoutcome').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'qocInvestigationOutcome', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      qocInvestigationOutcome: '', // Empty input for QOC Investigation Outcome
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.qocInvestigationOutcome}"?`
      : `Are you sure you want to inactivate the "${row.qocInvestigationOutcome}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'qocinvestigationoutcome', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('QOC Investigation Outcome updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating QOC Investigation Outcome!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('QOC Investigation Outcome:', row.qocInvestigationOutcome);

    // Check if QOC Investigation Outcome is empty
    if (!row.qocInvestigationOutcome) {
      this.snackBar.open('QOC Investigation Outcome is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate QOC Investigation Outcome
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.qocInvestigationOutcome?.toLowerCase() === row.qocInvestigationOutcome.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('QOC Investigation Outcome already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'qocinvestigationoutcome', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Investigation Outcome added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'qocinvestigationoutcome', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('QOC Investigation Outcome updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.qocInvestigationOutcome}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'qocinvestigationoutcome', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('QOC Investigation Outcome deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting QOC Investigation Outcome!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
