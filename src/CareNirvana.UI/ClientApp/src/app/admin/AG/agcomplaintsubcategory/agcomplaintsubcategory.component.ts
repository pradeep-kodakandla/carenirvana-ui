...
@Component({
  selector: 'app-agcomplaintsubcategory',  
  templateUrl: './agcomplaintsubcategory.component.html',
  styleUrl: './agcomplaintsubcategory.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcomplaintsubcategoryComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'complaintSubCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'complaintsubcategory').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'complaintSubCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      complaintSubCategory: '', // Empty input for Complaint Sub Category
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.complaintSubCategory}"?`
      : `Are you sure you want to inactivate the "${row.complaintSubCategory}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'complaintsubcategory', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Complaint Sub Category updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Complaint Sub Category!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Complaint Sub Category:', row.complaintSubCategory);

    // Check if Complaint Sub Category is empty
    if (!row.complaintSubCategory) {
      this.snackBar.open('Complaint Sub Category is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Complaint Sub Category
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.complaintSubCategory?.toLowerCase() === row.complaintSubCategory.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Complaint Sub Category already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'complaintsubcategory', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Sub Category added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'complaintsubcategory', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Sub Category updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.complaintSubCategory}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'complaintsubcategory', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Complaint Sub Category deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Complaint Sub Category!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
