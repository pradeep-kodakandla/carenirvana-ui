...
@Component({
  selector: 'app-agcoordinatortype',  
  templateUrl: './agcoordinatortype.component.html',
  styleUrl: './agcoordinatortype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcoordinatortypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'coordinatorType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'coordinatortype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'coordinatorType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      coordinatorType: '', // Empty input for Coordinator Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.coordinatorType}"?`
      : `Are you sure you want to inactivate the "${row.coordinatorType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'coordinatortype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Coordinator Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Coordinator Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Coordinator Type:', row.coordinatorType);

    // Check if Coordinator Type is empty
    if (!row.coordinatorType) {
      this.snackBar.open('Coordinator Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Coordinator Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.coordinatorType?.toLowerCase() === row.coordinatorType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Coordinator Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'coordinatortype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Coordinator Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'coordinatortype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Coordinator Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.coordinatorType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'coordinatortype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Coordinator Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Coordinator Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
