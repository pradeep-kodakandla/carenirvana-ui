...
@Component({
  selector: 'app-agcomplaintcredentials',  
  templateUrl: './agcomplaintcredentials.component.html',
  styleUrl: './agcomplaintcredentials.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgcomplaintcredentialsComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'complaintCredentials', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'complaintcredentials').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'complaintCredentials', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      complaintCredentials: '', // Empty input for Complaint Credentials
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.complaintCredentials}"?`
      : `Are you sure you want to inactivate the "${row.complaintCredentials}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'complaintcredentials', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Complaint Credentials updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Complaint Credentials!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Complaint Credentials:', row.complaintCredentials);

    // Check if Complaint Credentials is empty
    if (!row.complaintCredentials) {
      this.snackBar.open('Complaint Credentials is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Complaint Credentials
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.complaintCredentials?.toLowerCase() === row.complaintCredentials.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Complaint Credentials already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'complaintcredentials', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Credentials added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'complaintcredentials', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Complaint Credentials updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.complaintCredentials}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'complaintcredentials', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Complaint Credentials deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Complaint Credentials!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
