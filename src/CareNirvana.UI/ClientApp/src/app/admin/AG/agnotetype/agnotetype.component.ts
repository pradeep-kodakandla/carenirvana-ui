...
@Component({
  selector: 'app-agnotetype',
  templateUrl: './agnotetype.component.html',
  styleUrl: './agnotetype.component.css'
})
export class AgnotetypeComponent implements OnInit {
  ...
  loadData() {
    this.crudService.getData('ag', 'notetype').subscribe((response) => {
  ...
  saveEntry() {
    ...
    if (this.formMode === 'add') {
      ...
      // Save new entry
      this.crudService.addData('ag', 'notetype', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('ag', 'notetype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    if (confirm(message)) {
      ...
      this.crudService.updateData('ag', 'notetype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Note Type updated successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'notetype', id, 'current_user').subscribe(
      () => {
