...
@Component({
  selector: 'app-agparticipantrole',  
  templateUrl: './agparticipantrole.component.html',
  styleUrl: './agparticipantrole.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgparticipantroleComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'participantRole', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'participantrole').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'participantRole', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      participantRole: '', // Empty input for Participant Role
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.participantRole}"?`
      : `Are you sure you want to inactivate the "${row.participantRole}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'participantrole', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Participant Role updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Participant Role!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Participant Role:', row.participantRole);

    // Check if Participant Role is empty
    if (!row.participantRole) {
      this.snackBar.open('Participant Role is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Participant Role
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.participantRole?.toLowerCase() === row.participantRole.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Participant Role already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'participantrole', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Participant Role added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'participantrole', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Participant Role updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.participantRole}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'participantrole', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Participant Role deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Participant Role!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
