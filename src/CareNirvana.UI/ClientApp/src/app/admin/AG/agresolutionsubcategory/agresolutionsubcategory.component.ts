...
@Component({
  selector: 'app-agresolutionsubcategory',  
  templateUrl: './agresolutionsubcategory.component.html',
  styleUrl: './agresolutionsubcategory.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class AgresolutionsubcategoryComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'resolutionSubCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('ag', 'resolutionsubcategory').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'resolutionSubCategory', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      resolutionSubCategory: '', // Empty input for Resolution Sub Category
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.resolutionSubCategory}"?`
      : `Are you sure you want to inactivate the "${row.resolutionSubCategory}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('ag', 'resolutionsubcategory', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Resolution Sub Category updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Resolution Sub Category!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Resolution Sub Category:', row.resolutionSubCategory);

    // Check if Resolution Sub Category is empty
    if (!row.resolutionSubCategory) {
      this.snackBar.open('Resolution Sub Category is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Resolution Sub Category
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.resolutionSubCategory?.toLowerCase() === row.resolutionSubCategory.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Resolution Sub Category already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('ag', 'resolutionsubcategory', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Resolution Sub Category added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('ag', 'resolutionsubcategory', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Resolution Sub Category updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.resolutionSubCategory}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('ag', 'resolutionsubcategory', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Resolution Sub Category deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Resolution Sub Category!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
