...
@Component({
  selector: 'app-cmgoalclass',
  templateUrl: './cmgoalclass.component.html',
  styleUrl: './cmgoalclass.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmgoalclassComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'goalClass', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'goalclass').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'goalClass', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      goalClass: '', // Empty input for Goal Class
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.goalClass}"?`
      : `Are you sure you want to inactivate the "${row.goalClass}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'goalclass', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Goal Class updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Goal Class!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Goal Class:', row.goalClass);

    // Check if Goal Class is empty
    if (!row.goalClass) {
      this.snackBar.open('Goal Class is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Goal Class
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.goalClass?.toLowerCase() === row.goalClass.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Goal Class already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'goalclass', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Goal Class added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'goalclass', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Goal Class updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.goalClass}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'goalclass', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Goal Class deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Goal Class!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
