...
@Component({
  selector: 'app-cmsexualorientation',
  templateUrl: './cmsexualorientation.component.html',
  styleUrl: './cmsexualorientation.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmsexualorientationComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'sexualOrientation', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'sexualorientation').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'sexualOrientation', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      sexualOrientation: '', // Empty input for Sexual Orientation
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.sexualOrientation}"?`
      : `Are you sure you want to inactivate the "${row.sexualOrientation}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'sexualorientation', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Sexual Orientation updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Sexual Orientation!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Sexual Orientation:', row.sexualOrientation);

    // Check if Sexual Orientation is empty
    if (!row.sexualOrientation) {
      this.snackBar.open('Sexual Orientation is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Sexual Orientation
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.sexualOrientation?.toLowerCase() === row.sexualOrientation.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Sexual Orientation already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'sexualorientation', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Sexual Orientation added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'sexualorientation', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Sexual Orientation updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.sexualOrientation}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'sexualorientation', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Sexual Orientation deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Sexual Orientation!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
