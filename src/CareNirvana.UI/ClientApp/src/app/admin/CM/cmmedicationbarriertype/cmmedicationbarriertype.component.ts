...
@Component({
  selector: 'app-cmmedicationbarriertype',
  templateUrl: './cmmedicationbarriertype.component.html',
  styleUrl: './cmmedicationbarriertype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmmedicationbarriertypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'medicationBarrierType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'medicationbarriertype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'medicationBarrierType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      medicationBarrierType: '', // Empty input for Medication Barrier Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.medicationBarrierType}"?`
      : `Are you sure you want to inactivate the "${row.medicationBarrierType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'medicationbarriertype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Medication Barrier Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Medication Barrier Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Medication Barrier Type:', row.medicationBarrierType);

    // Check if Medication Barrier Type is empty
    if (!row.medicationBarrierType) {
      this.snackBar.open('Medication Barrier Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Medication Barrier Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.medicationBarrierType?.toLowerCase() === row.medicationBarrierType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Medication Barrier Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'medicationbarriertype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Barrier Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'medicationbarriertype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Barrier Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.medicationBarrierType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'medicationbarriertype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Medication Barrier Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Medication Barrier Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
