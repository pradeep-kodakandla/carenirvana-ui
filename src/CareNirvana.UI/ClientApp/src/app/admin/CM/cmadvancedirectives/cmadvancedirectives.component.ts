...
@Component({
  selector: 'app-cmadvancedirectives',
  templateUrl: './cmadvancedirectives.component.html',
  styleUrl: './cmadvancedirectives.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmadvancedirectivesComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'advanceDirectives', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'advancedirectives').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'advanceDirectives', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      advanceDirectives: '', // Empty input for Advance Directives
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.advanceDirectives}"?`
      : `Are you sure you want to inactivate the "${row.advanceDirectives}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'advancedirectives', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Advance Directives updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Advance Directives!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Advance Directives:', row.advanceDirectives);

    // Check if Advance Directives is empty
    if (!row.advanceDirectives) {
      this.snackBar.open('Advance Directives is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Advance Directives
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.advanceDirectives?.toLowerCase() === row.advanceDirectives.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Advance Directives already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'advancedirectives', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Advance Directives added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'advancedirectives', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Advance Directives updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.advanceDirectives}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'advancedirectives', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Advance Directives deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Advance Directives!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
