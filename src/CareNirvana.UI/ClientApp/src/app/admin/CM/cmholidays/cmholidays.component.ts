...
@Component({
  selector: 'app-cmholidays',
  templateUrl: './cmholidays.component.html',
  styleUrl: './cmholidays.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmholidaysComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'holidays', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'holidays').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'holidays', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      holidays: '', // Empty input for Holidays
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.holidays}"?`
      : `Are you sure you want to inactivate the "${row.holidays}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'holidays', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Holidays updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Holidays!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Holidays:', row.holidays);

    // Check if Holidays is empty
    if (!row.holidays) {
      this.snackBar.open('Holidays is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.holidays?.toLowerCase() === row.holidays.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Holidays already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'holidays', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Holidays added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'holidays', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Holidays updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.holidays}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'holidays', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Holidays deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Holidays!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
