...
@Component({
  selector: 'app-cmcareplanduration',
  templateUrl: './cmcareplanduration.component.html',
  styleUrl: './cmcareplanduration.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmcareplandurationComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'carePlanDuration', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'careplanduration').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'carePlanDuration', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      carePlanDuration: '', // Empty input for Care Plan Duration
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.carePlanDuration}"?`
      : `Are you sure you want to inactivate the "${row.carePlanDuration}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'careplanduration', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Care Plan Duration updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Care Plan Duration!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Care Plan Duration:', row.carePlanDuration);

    // Check if Care Plan Duration is empty
    if (!row.carePlanDuration) {
      this.snackBar.open('Care Plan Duration is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Care Plan Duration
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.carePlanDuration?.toLowerCase() === row.carePlanDuration.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Care Plan Duration already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'careplanduration', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Care Plan Duration added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'careplanduration', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Care Plan Duration updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.carePlanDuration}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'careplanduration', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Care Plan Duration deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Care Plan Duration!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
