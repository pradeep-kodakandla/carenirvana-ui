...
@Component({
  selector: 'app-cmcareplanmatrix',
  templateUrl: './cmcareplanmatrix.component.html',
  styleUrl: './cmcareplanmatrix.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmcareplanmatrixComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'carePlanMatrix', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'careplanmatrix').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'carePlanMatrix', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      carePlanMatrix: '', // Empty input for Address Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.carePlanMatrix}"?`
      : `Are you sure you want to inactivate the "${row.carePlanMatrix}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'careplanmatrix', row.id, row).subscribe(() => {
        this.loadData();
  ...
  saveEntry(row: any) {
    ...
    console.log('Address Type:', row.carePlanMatrix);

    // Check if Address Type is empty
    if (!row.carePlanMatrix) {
      this.snackBar.open('Address Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.carePlanMatrix?.toLowerCase() === row.carePlanMatrix.toLowerCase() &&
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'careplanmatrix', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('cm', 'careplanmatrix', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Address Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.carePlanMatrix}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'careplanmatrix', id, 'current_user').subscribe(
      () => {
