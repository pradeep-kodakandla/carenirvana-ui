...
@Component({
  selector: 'app-cmmedicationfrequency',
  templateUrl: './cmmedicationfrequency.component.html',
  styleUrl: './cmmedicationfrequency.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmmedicationfrequencyComponent implements OnInit {
  ...
  loadData() {
    this.crudService.getData('cm', 'medicationfrequency').subscribe((response) => {
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    if (confirm(message)) {
      ...
      this.crudService.updateData('cm', 'medicationfrequency', row.id, row).subscribe(() => {
        this.loadData();
  ...
  saveEntry(row: any) {
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'medicationfrequency', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('cm', 'medicationfrequency', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Frequency updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'medicationfrequency', id, 'current_user').subscribe(
      () => {
