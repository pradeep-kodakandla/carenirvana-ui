...
@Component({
  selector: 'app-cmcondition',
  templateUrl: './cmcondition.component.html',
  styleUrl: './cmcondition.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmconditionComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'condition', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'condition').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'condition', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      condition: '', // Empty input for Condition
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.condition}"?`
      : `Are you sure you want to inactivate the "${row.condition}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'condition', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Condition updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Condition!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    if (!row.condition) {
      this.snackBar.open('Condition is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.condition?.toLowerCase() === row.condition.toLowerCase() &&
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'condition', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('cm', 'condition', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Condition updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.condition}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'condition', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Condition deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Condition!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
