...
@Component({
  selector: 'app-cmcauseofdeath',
  templateUrl: './cmcauseofdeath.component.html',
  styleUrl: './cmcauseofdeath.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmcauseofdeathComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'causeOfDeath', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'causeofdeath').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'causeOfDeath', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      causeOfDeath: '', // Empty input for Cause Of Death
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.causeOfDeath}"?`
      : `Are you sure you want to inactivate the "${row.causeOfDeath}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'causeofdeath', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Cause Of Death updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Cause Of Death!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Cause Of Death:', row.causeOfDeath);

    // Check if Cause Of Death is empty
    if (!row.causeOfDeath) {
      this.snackBar.open('Cause Of Death is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Cause Of Death
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.causeOfDeath?.toLowerCase() === row.causeOfDeath.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Cause Of Death already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'causeofdeath', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Cause Of Death added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'causeofdeath', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Cause Of Death updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.causeOfDeath}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'causeofdeath', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Cause Of Death deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Cause Of Death!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
