...
@Component({
  selector: 'app-cmstrengthtype',
  templateUrl: './cmstrengthtype.component.html',
  styleUrl: './cmstrengthtype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmstrengthtypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'strengthType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'strengthtype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'strengthType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      strengthType: '', // Empty input for Strength Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.strengthType}"?`
      : `Are you sure you want to inactivate the "${row.strengthType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'strengthtype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Strength Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Strength Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Strength Type:', row.strengthType);

    // Check if Strength Type is empty
    if (!row.strengthType) {
      this.snackBar.open('Strength Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Strength Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.strengthType?.toLowerCase() === row.strengthType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Strength Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'strengthtype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Strength Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'strengthtype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Strength Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.strengthType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'strengthtype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Strength Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Strength Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
