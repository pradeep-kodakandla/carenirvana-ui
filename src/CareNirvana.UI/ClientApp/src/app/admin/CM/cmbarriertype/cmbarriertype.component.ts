...
@Component({
  selector: 'app-cmbarriertype',
  templateUrl: './cmbarriertype.component.html',
  styleUrl: './cmbarriertype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmbarriertypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'barrierType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'barriertype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'barrierType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      barrierType: '', // Empty input for Barrier Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.barrierType}"?`
      : `Are you sure you want to inactivate the "${row.barrierType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'barriertype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Barrier Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Barrier Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Barrier Type:', row.barrierType);

    // Check if Barrier Type is empty
    if (!row.barrierType) {
      this.snackBar.open('Barrier Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Barrier Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.barrierType?.toLowerCase() === row.barrierType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Barrier Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'barriertype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Barrier Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'barriertype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Barrier Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.barrierType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'barriertype', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Barrier Type deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Barrier Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
