...
@Component({
  selector: 'app-cmevacuationzone',
  templateUrl: './cmevacuationzone.component.html',
  styleUrl: './cmevacuationzone.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmevacuationzoneComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'evacuationZone', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'evacuationzone').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'evacuationZone', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      evacuationZone: '', // Empty input for Evacuation Zone
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.evacuationZone}"?`
      : `Are you sure you want to inactivate the "${row.evacuationZone}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'evacuationzone', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Evacuation Zone updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Evacuation Zone!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Evacuation Zone:', row.evacuationZone);

    // Check if Evacuation Zone is empty
    if (!row.evacuationZone) {
      this.snackBar.open('Evacuation Zone is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Evacuation Zone
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.evacuationZone?.toLowerCase() === row.evacuationZone.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Evacuation Zone already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'evacuationzone', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Evacuation Zone added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'evacuationzone', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Evacuation Zone updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.evacuationZone}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'evacuationzone', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Evacuation Zone deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Evacuation Zone!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
