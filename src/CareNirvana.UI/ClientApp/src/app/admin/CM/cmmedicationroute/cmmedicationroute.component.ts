...
@Component({
  selector: 'app-cmmedicationroute',
  templateUrl: './cmmedicationroute.component.html',
  styleUrl: './cmmedicationroute.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmmedicationrouteComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'medicationRoute', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'medicationroute').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'medicationRoute', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      medicationRoute: '', // Empty input for Medication Route
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.medicationRoute}"?`
      : `Are you sure you want to inactivate the "${row.medicationRoute}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'medicationroute', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Medication Route updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Medication Route!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Medication Route:', row.medicationRoute);

    // Check if Medication Route is empty
    if (!row.medicationRoute) {
      this.snackBar.open('Medication Route is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Medication Route
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.medicationRoute?.toLowerCase() === row.medicationRoute.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Medication Route already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'medicationroute', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Route added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'medicationroute', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Route updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.medicationRoute}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'medicationroute', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Medication Route deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Medication Route!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
