...
@Component({
  selector: 'app-cmappointmenttype',
  templateUrl: './cmappointmenttype.component.html',
  styleUrl: './cmappointmenttype.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmappointmenttypeComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'appointmentType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'appointmenttype').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'appointmentType', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      appointmentType: '', // Empty input for Appointment Type
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.appointmentType}"?`
      : `Are you sure you want to inactivate the "${row.appointmentType}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'appointmenttype', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Appointment Type updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Appointment Type!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Appointment Type:', row.appointmentType);

    // Check if Appointment Type is empty
    if (!row.appointmentType) {
      this.snackBar.open('Appointment Type is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Appointment Type
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.appointmentType?.toLowerCase() === row.appointmentType.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Appointment Type already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'appointmenttype', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Appointment Type added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'appointmenttype', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Appointment Type updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.appointmentType}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'appointmenttype', id, 'current_user').subscribe(
      () => {
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Appointment Type!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
