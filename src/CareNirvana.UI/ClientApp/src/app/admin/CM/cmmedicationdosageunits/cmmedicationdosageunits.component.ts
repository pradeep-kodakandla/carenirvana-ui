...
@Component({
  selector: 'app-cmmedicationdosageunits',
  templateUrl: './cmmedicationdosageunits.component.html',
  styleUrl: './cmmedicationdosageunits.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmmedicationdosageunitsComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'medicationDosageUnits', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'medicationdosageunits').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'medicationDosageUnits', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      medicationDosageUnits: '', // Empty input for Medication Dosage Units
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.medicationDosageUnits}"?`
      : `Are you sure you want to inactivate the "${row.medicationDosageUnits}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'medicationdosageunits', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Medication Dosage Units updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Medication Dosage Units!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Medication Dosage Units:', row.medicationDosageUnits);

    // Check if Medication Dosage Units is empty
    if (!row.medicationDosageUnits) {
      this.snackBar.open('Medication Dosage Units is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Medication Dosage Units
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.medicationDosageUnits?.toLowerCase() === row.medicationDosageUnits.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Medication Dosage Units already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'medicationdosageunits', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Dosage Units added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'medicationdosageunits', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Medication Dosage Units updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.medicationDosageUnits}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'medicationdosageunits', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Medication Dosage Units deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Medication Dosage Units!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
