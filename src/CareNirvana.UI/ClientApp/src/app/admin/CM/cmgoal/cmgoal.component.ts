...
@Component({
  selector: 'app-cmgoal',
  templateUrl: './cmgoal.component.html',
  styleUrl: './cmgoal.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmgoalComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'goal', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'goal').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'goal', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.goal}"?`
      : `Are you sure you want to inactivate the "${row.goal}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'goal', row.id, row).subscribe(() => {
        this.loadData();
  ...
  saveEntry(row: any) {
    ...
    if (!row.goal) {
      this.snackBar.open('Goal is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.goal?.toLowerCase() === row.goal.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Goal already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'goal', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('cm', 'goal', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Goal updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.goal}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'goal', id, 'current_user').subscribe(
      () => {
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Goal!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
