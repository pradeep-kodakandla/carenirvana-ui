...
@Component({
  selector: 'app-cmproblem',
  templateUrl: './cmproblem.component.html',
  styleUrl: './cmproblem.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmproblemComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'problem', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'problem').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'problem', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      problem: '', // Empty input for Problem
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.problem}"?`
      : `Are you sure you want to inactivate the "${row.problem}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'problem', row.id, row).subscribe(() => {
        this.loadData();
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Problem!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    if (!row.problem) {
      this.snackBar.open('Problem is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.problem?.toLowerCase() === row.problem.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Problem already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'problem', this.selectedEntry).subscribe(
        () => {
      ...
      this.crudService.updateData('cm', 'problem', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Problem updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.problem}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'problem', id, 'current_user').subscribe(
      () => {
