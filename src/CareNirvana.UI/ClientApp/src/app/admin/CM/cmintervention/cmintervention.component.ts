...
@Component({
  selector: 'app-cmintervention',
  templateUrl: './cmintervention.component.html',
  styleUrl: './cmintervention.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CminterventionComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'intervention', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'intervention').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'intervention', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      intervention: '', // Empty input for Intervention
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.intervention}"?`
      : `Are you sure you want to inactivate the "${row.intervention}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'intervention', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Intervention updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Intervention!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Intervention:', row.intervention);

    // Check if Intervention is empty
    if (!row.intervention) {
      this.snackBar.open('Intervention is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.intervention?.toLowerCase() === row.intervention.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Intervention already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'intervention', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Intervention added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'intervention', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Intervention updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.intervention}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'intervention', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Intervention deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Intervention!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
