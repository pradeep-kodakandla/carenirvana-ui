...
@Component({
  selector: 'app-cmcareplanstatus',
  templateUrl: './cmcareplanstatus.component.html',
  styleUrl: './cmcareplanstatus.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmcareplanstatusComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'carePlanStatus', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'careplanstatus').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'carePlanStatus', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      carePlanStatus: '', // Empty input for Care Plan Status
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.carePlanStatus}"?`
      : `Are you sure you want to inactivate the "${row.carePlanStatus}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'careplanstatus', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Care Plan Status updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Care Plan Status!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Care Plan Status:', row.carePlanStatus);

    // Check if Care Plan Status is empty
    if (!row.carePlanStatus) {
      this.snackBar.open('Care Plan Status is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Care Plan Status
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.carePlanStatus?.toLowerCase() === row.carePlanStatus.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Care Plan Status already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'careplanstatus', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Care Plan Status added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'careplanstatus', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Care Plan Status updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.carePlanStatus}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'careplanstatus', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Care Plan Status deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Care Plan Status!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
