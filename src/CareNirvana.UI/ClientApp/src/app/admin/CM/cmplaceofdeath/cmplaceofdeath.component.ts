...
@Component({
  selector: 'app-cmplaceofdeath',
  templateUrl: './cmplaceofdeath.component.html',
  styleUrl: './cmplaceofdeath.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CmplaceofdeathComponent implements OnInit {
  dataSource = new MatTableDataSource<any>();
  displayedColumns: string[] = ['id', 'placeOfDeath', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  loadData() {
    this.crudService.getData('cm', 'placeofdeath').subscribe((response) => {
  ...
  updateDisplayedColumns() {
    ...
    this.displayedColumns = ['id', 'placeOfDeath', 'activeFlag', 'createdBy', 'createdOn', 'actions'];
  ...
  addRow(): void {
    const newRow = {
      ...
      placeOfDeath: '', // Empty input for Place Of Death
  ...
  saveRow(event: MatCheckboxChange, row: any): void {
    ...
    const message = isChecked
      ? `Are you sure you want to activate the "${row.placeOfDeath}"?`
      : `Are you sure you want to inactivate the "${row.placeOfDeath}"?`;

    if (confirm(message)) {
      row.activeFlag = event.checked;
      console.log('Checkbox checked:', event.checked);
      this.crudService.updateData('cm', 'placeofdeath', row.id, row).subscribe(() => {
        this.loadData();
        this.snackBar.open('Place Of Death updated successfully!', 'Close', {
        ...
        (error) => {
          ...
          this.snackBar.open('Error updating Place Of Death!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000
          });
  ...
  saveEntry(row: any) {
    ...
    console.log('Place Of Death:', row.placeOfDeath);

    // Check if Place Of Death is empty
    if (!row.placeOfDeath) {
      this.snackBar.open('Place Of Death is required!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    // Check for duplicate Place Of Death
    const isDuplicate = this.dataSource.data.some(
      (entry) =>
        entry.placeOfDeath?.toLowerCase() === row.placeOfDeath.toLowerCase() &&
        entry.id !== (this.selectedEntry?.id || null) // Exclude the current entry being edited
    );

    if (isDuplicate) {
      this.snackBar.open('Place Of Death already exists!', 'Close', {
        horizontalPosition: 'center',
        verticalPosition: 'top',
        duration: 5000
      });
    ...
    if (this.formMode === 'add') {
      ...
      this.crudService.addData('cm', 'placeofdeath', this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Place Of Death added successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
            duration: 5000,
      ...
      this.crudService.updateData('cm', 'placeofdeath', this.selectedEntry.id, this.selectedEntry).subscribe(
        () => {
          this.snackBar.open('Place Of Death updated successfully!', 'Close', {
            horizontalPosition: 'center',
            verticalPosition: 'top',
  ...
  confirmDelete(element: any): void {
    const confirmed = window.confirm(`Are you sure you want to delete the "${element.placeOfDeath}"?`);
    if (confirmed) {
  ...
  deleteRow(id: number) {
    this.crudService.deleteData('cm', 'placeofdeath', id, 'current_user').subscribe(
      () => {
        this.snackBar.open('Place Of Death deleted successfully!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
      ...
      (error) => {
        ...
        this.snackBar.open('Error deleting Place Of Death!', 'Close', {
          horizontalPosition: 'center',
          verticalPosition: 'top',
          duration: 5000
        });
